...
# API Routes:
```bash
  #With apis need auth. please add token in to Authorization key (Headers): Format; `Bearer ${token}`

  #payload Login: object with keys [email, password]
  #payload Register: object with keys [email, password, confirmPassword]
```
+--------+-------------------------+
  Method | URI
+--------+-------------------------+
